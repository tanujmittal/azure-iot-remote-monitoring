...
namespace Microsoft.Azure.Devices.Applications.RemoteMonitoring.Simulator.WebJob.CGMDevice.Devices.Factory
{
    public class CBGMDeviceFactory : IDeviceFactory
    {
        ...
        {
            var device = new CBGMDevice(logger, transportFactory, telemetryFactory, configurationProvider);
            device.Init(config);
            return device;
        }
    }
